...
import { getAnalytics } from '../controllers/dashboard.controllers.js'; // Import the new controller
...
router.get('/customers', protectRoute, async (req, res) => {
    try {
        const seller = await Seller.findById(req.user._id).populate('customers', 'fullname email');
        res.status(200).json(seller.customers);
    } catch (error) { res.status(500).json({ message: "Server Error" }); }
});
