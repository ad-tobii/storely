...
import { Link, useNavigate } from "react-router-dom" // Use react-router-dom's Link/navigate
...
const ArtisanElegantApp = ({ editMode = false, storeSettings = {}, seller = {}, sellerId }) => {
  ...
  const renderPage = () => {
    ...
    // Note: The 'cart' case is removed, as it's handled by the global /cart route
    switch (currentPage) {
      ...
      <nav className="p-4 shadow-sm" style={{ backgroundColor: mergedSettings.secondaryColor }}>
        <div className="container mx-auto flex flex-col md:flex-row justify-between items-center">
          <div className="text-2xl font-bold mb-4 md:mb-0 cursor-pointer" onClick={() => navigate("home")} style={{ color: mergedSettings.primaryTextColor }}>
            {seller.storeLogoUrl ? <img src={seller.storeLogoUrl} alt={`${seller.storename} logo`} className="h-10" /> : (seller.storename || "Artisan Elegant")}
          </div>
